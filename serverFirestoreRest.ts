...
// URL base: se usa únicamente para llamadas HTTP GET/POST.
...
// Nombre de recurso Firestore: se usa dentro de writes[].update.name.
// IMPORTANTE: aquí NO puede ir https://firestore.googleapis.com/...
const RESOURCE_BASE = `projects/${PROJECT_ID}/databases/${DATABASE_ID}`;
...
export function getAuthenticatedClaims(): { uid?: string; email?: string } {
  ...
  return {
    uid: typeof payload.user_id === "string"
      ? payload.user_id
...
function resourceDocumentPath(collectionName: string, id: string): string {
  return `${RESOURCE_BASE}/documents/${collectionName}/${id}`;
}
...
async function commitWithRetry(writes: any[], context: string, maxAttempts = 3): Promise<any> {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      const retryable =
        /_(408|429|5\d\d):/.test(lastError.message) ||
        /fetch|network|ECONN|ETIMEDOUT/i.test(lastError.message);
...
export async function readHomeDocument(homeCode: string): Promise<{ exists: boolean; data: any; updateTime?: string }> {
  ...
  if (!code) return { exists: false, data: null };

  const response = await firestoreFetch(documentPath("nests", code), { method: "GET" });
  if (response.status === 404) return { exists: false, data: null };
  if (!response.ok) {
    throw new Error(`FIRESTORE_READ_${response.status}:${await readResponseText(response)}`);
  }
...
export async function readAccountHomeIndex(uid: string): Promise<{ homeCode: string | null }> {
  ...
  if (!response.ok) {
    throw new Error(`ACCOUNT_HOME_READ_${response.status}:${await readResponseText(response)}`);
  }

  const payload = await response.json();
  const data = decodeFields(payload.fields);
  return {
    homeCode: typeof data.homeCode === "string" ? data.homeCode.trim().toUpperCase() : null
  };
}
...
export async function writeAccountHomeIndex(uid: string, homeCode: string): Promise<void> {
  ...
  // Firestore Commit API requiere el resource name, NO una URL HTTP.
  const documentName = resourceDocumentPath("account_homes", cleanUid);
...
function buildAuthorizationMetadata(existing: any, candidateData: any): { authorizedUids: string[]; authorizedEmails: string[] } {
  ...
  const uids = new Set<string>(
    Array.isArray(existing?.authorizedUids)
      ? existing.authorizedUids.map(String)
      : []
  );
  const emails = new Set<string>(
    Array.isArray(existing?.authorizedEmails)
      ? existing.authorizedEmails.map((v: any) => String(v).trim().toLowerCase())
      : []
  );
  ...
  if (Array.isArray(users)) {
    for (const user of users) {
      if (typeof user?.authUid === "string" && user.authUid.trim()) {
        uids.add(user.authUid.trim());
      }
...
export async function writeHomeDocument(homeCode: string, data: any, expectedRevision?: number): Promise<number> {
  const code = String(homeCode || "").trim().toUpperCase();
  if (!code) throw new Error("HOME_CODE_REQUIRED");

  const existing = await readHomeDocument(code);
  ...
  if (actualRevision !== expected) {
    throw new Error(`SYNC_CONFLICT:${code}:expected=${expected}:actual=${actualRevision}`);
  }
  ...
  // Firestore Commit API requiere:
  // projects/{project}/databases/{database}/documents/{collection}/{doc}
  const documentName = resourceDocumentPath("nests", code);
  ...
  try {
    await commitWithRetry([
      {
        update: {
          name: `${documentName}/history/${nextRevision}`,
          fields: encodeFields({
            homeCode: code,
            revision: nextRevision,
            savedAt,
            actorUserId: getAuthenticatedClaims().uid || "autenticado",
            source: "astro-hogar",
    ...
    console.warn("[Firestore Sync] No se pudo guardar el historial inicial:", historyError);
  }

  const currentUid = getAuthenticatedClaims().uid;
  if (currentUid) {
    try {
      await writeAccountHomeIndex(currentUid, code);
    } catch (indexError) {
      console.warn("[Account Home Index] No se pudo actualizar el índice:", indexError);
    }
  }
