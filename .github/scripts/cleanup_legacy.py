...
new_save = '''export function saveToDisk() {\n  if (process.env.NODE_ENV !== "production") {\n    try { fs.writeFileSync(DB_FILE, JSON.stringify(multiStore, null, 2), "utf8"); }\n    catch (err) { console.error("Error saving local development database:", err); }\n  }\n  hasUnsyncedChanges = true;'''
...
if first != -1:
    s = s[:first + len(sentinel)] + s[first + len(sentinel):].replace(sentinel, '', 1)
s = s.replace('multiStore[cleanCode].home.name = `Hogar de Mafe y Benjamin`;', 'multiStore[cleanCode].home.name = "";')
server.write_text(s, encoding="utf-8")

app = ROOT / "src/App.tsx"
a = app.read_text(encoding="utf-8")
a = a.replace('const code = home?.code || localStorage.getItem("astro_home_code") || "HOGARPELUDO";', 'const code = home?.code || localStorage.getItem("astro_home_code") || "";')
...
print("Legacy household fallbacks cleaned.")
