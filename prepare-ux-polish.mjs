...
// Make the environment strip compact on phones and smaller on desktop.
patchText("src/components/HomeEnvironmentStrip.tsx", (source) => {
  return source
    .replace('className="space-y-3"', 'className="space-y-2"', 1)
    .replace('className="rounded-3xl border-2 border-[#E7E2D5] bg-white/95 p-4 sm:p-5 shadow-sm"', 'className="rounded-2xl border border-[#E7E2D5] bg-white/95 p-2.5 sm:p-3 shadow-sm"', 1)
    .replace('className="grid grid-cols-1 xl:grid-cols-5 gap-3"', 'className="grid grid-cols-2 lg:grid-cols-4 gap-2"', 1)
    .replace('className="xl:col-span-2 rounded-2xl bg-[#FAF7F2] p-4"', 'className="col-span-2 lg:col-span-1 rounded-2xl bg-[#FAF7F2] p-2.5"', 1)
    .replace('className="rounded-2xl border border-blue-100 bg-blue-50/70 p-4"', 'className="rounded-2xl border border-blue-100 bg-blue-50/70 p-2.5"', 1)
    .replace('className="rounded-2xl border border-amber-100 bg-amber-50/70 p-4"', 'className="rounded-2xl border border-amber-100 bg-amber-50/70 p-2.5"', 1)
    .replace('className="rounded-2xl border border-violet-100 bg-violet-50/70 p-4"', 'className="rounded-2xl border border-violet-100 bg-violet-50/70 p-2.5"', 1)
    .replace('className="rounded-2xl border border-emerald-200 bg-emerald-50/70 p-4 text-left', 'className="rounded-2xl border border-emerald-200 bg-emerald-50/70 p-2.5 text-left', 1)
    .replace('className="text-base font-black text-[#2C2723] capitalize mt-1"', 'className="text-sm font-black text-[#2C2723] capitalize mt-0.5 leading-tight"', 1)
    .replace('className="text-2xl font-black text-[#2C2723] mt-1 flex items-center gap-2"', 'className="text-lg font-black text-[#2C2723] mt-0.5 flex items-center gap-1.5"', 1)
    .replace('className="text-xl font-black text-[#2C2723] mt-1"', 'className="text-base font-black text-[#2C2723] mt-0.5"', 1)
    .replace('className="text-4xl">🐾</div>', 'className="text-2xl">🐾</div>', 1)
    .replace('className="rounded-3xl border-2 border-amber-200 bg-amber-50/90 p-4 shadow-sm"', 'className="rounded-2xl border border-amber-200 bg-amber-50/90 p-2.5 shadow-sm"', 1);
});
...
patchText("src/components/HomeEnvironmentStrip.tsx", (source) => {
  ...
  const next = `  async function requestNotifications() {\n    if (!("Notification" in window)) {\n      setNotificationStatus("unsupported");\n      return;\n    }\n    if (Notification.permission === "denied") {\n      setNotificationStatus("denied");\n      return;\n    }\n    try {\n      // Must happen directly from the user's click; do not await another async task first.\n      const permission = await Notification.requestPermission();`;
...
// Make the location prompt equally explicit when the browser has blocked it.
patchText("src/components/HomeEnvironmentStrip.tsx", (source) => {
  const old = `{locationStatus !== "granted" && <button type="button" onClick={requestLocation}`;
  const next = `{locationStatus !== "granted" && <button type="button" onClick={requestLocation}`;
  return source.replace(old, next, 1);
});
...
console.log("[AstroHogar UX] Voice, permissions, environment density and PWA install polish prepared.");
