...
export default function HomeLunarPanel({ moonInfo, liveMoon, city = "Bogotá" }: { moonInfo?: MoonSource; liveMoon?: MoonSource; city?: string }) {
  ...
  const illumination = Number(model.illuminationPct ?? Math.round(((1 - Math.cos((age / SYNODIC_MONTH) * Math.PI * 2)) / 2) * 100));
  const now = new Date();
  ...
  const nextNewMoon = new Date(now.getTime() + (SYNODIC_MONTH - age) * 86400000);

  const stageDates = useMemo(() => STAGES.map(stage => dateForAge(now, stage.age)), []);
  const event = { name: "Eclipse lunar penumbral", date: "20 feb 2027", note: `Evento para seguir desde ${city}. Milo puede recordarlo cuando se acerque.` };
      ...
      <div className="space-y-3 relative">
        ...
        <div className="bg-purple-950/70 p-3.5 rounded-2xl border border-purple-800/60 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-base font-black text-amber-200 mt-1">{currentStage.long} {currentStage.icon}</p>
            <p className="text-[10.5px] text-purple-200/90">{illumination}% iluminada · {age.toFixed(1)} días de edad</p>
          </div>
          <div className="w-14 h-14 rounded-full bg-purple-900/50 border border-purple-700/60 flex items-center justify-center text-3xl shrink-0">{currentStage.icon}</div>
        </div>
        ...
        <div className="bg-purple-950/50 p-3 rounded-2xl border border-purple-800/50 space-y-2">
          ...
          <div className="relative pt-4 px-1 overflow-x-auto">
                ...
                {STAGES.map((stage, index) => {
                  ...
                  return (
                    <div key={stage.key} className="min-w-0 text-center">
                      ...
                      <p className={(active ? "text-amber-200 font-black " : "text-purple-300 ") + "text-[7px] leading-tight mt-1"}>{stage.short}</p>
                      <p className="text-[6.5px] text-purple-400 mt-0.5">{localDateLabel(stageDates[index])}</p>
                    </div>
                  );
                })}
              </div>
